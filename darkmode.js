(() => {
  ...
  function ensureMaterialIcons() {
    return new Promise(resolve => {
      ...
      if (!link) {
        ...
        link.onerror = () => { console.warn('Material Icons failed. Falling back to SVG.'); resolve(); };
        ...
        if (document.fonts?.check) {
          setTimeout(() => {
            try { materialIconsLoaded = document.fonts.check('24px "Material Icons"'); } catch {}
            done();
          }, 0);
        } else done();
      }
  ...
  function injectCSS() {
    ...
    const css = `
      ...
      body.dark-mode .topnav { background-color: #222 !important; color: #ddd !important; }
      body.dark-mode .menu-icon { color: #ddd !important; }
      body.dark-mode .logo { color: #ddd !important; }
      body.dark-mode .topnav-right { background-color: #222 !important; color: #ddd !important; }
      body.dark-mode .accordion-toggle { color: #ddd !important; }
      body.dark-mode .accordion-content { background-color: #222 !important; }
      body.dark-mode .accordion-content a { color: #ddd !important; border-top: 1px solid rgba(255,255,255,.15) !important; }
      body.dark-mode .clickable-section { background-color: #c52430 !important; border-top: 1px solid #932028 !important; color: #fff !important; }
      body.dark-mode .clickable-section:hover,
      body.dark-mode .clickable-section:focus { background-color: #b21f2b !important; }
    `;
  ...
  /* --------- Persistence helpers --------- */
  const KEYS = ['theme', 'darkmode-enabled', 'darkMode'];

  function getStoredTheme() {
    for (const k of KEYS) {
      const v = localStorage.getItem(k);
      if (v === 'dark' || v === 'true' || v === 'enabled') return 'dark';
      if (v === 'light' || v === 'false' || v === 'disabled') return 'light';
    }
    return null;
  }
  ...
  function insertToggle(topnav) {
    ...
    if (menuIcon && menuIcon.parentElement === topnav) topnav.insertBefore(toggleBtn,menuIcon);
    else topnav.appendChild(toggleBtn);
  ...
  function initTheme() {
    ...
    if(!stored && window.matchMedia){
      ...
      mq.addEventListener?.('change',e=>applyMode(e.matches?'dark':'light'));
    }
  }
  ...
  async function boot() {
    ...
    usingMaterial = materialIconsLoaded;
    injectCSS();
